...
pub struct Config {
    ...
    /// Per-label BM25 field filters. Key = node label (e.g. "Episode"),
    /// Value = field names to index (e.g. ["content"]). If a label is absent,
    /// all fields are indexed (backward compatible).
    #[serde(default)]
    pub bm25_field_filters: Option<HashMap<String, Vec<String>>>,
}
        ...
        r#"
{
    ...
    pub fn fmt_with_schema(
        ...
        writeln!(f, "bm25_field_filters: None,")?;
        writeln!(f, "}})")?;
        writeln!(f, "}}")?;
        Ok(())
    }
