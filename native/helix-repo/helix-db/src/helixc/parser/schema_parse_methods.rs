...
impl HelixParser {
    ...
    pub(super) fn parse_field_def(
        ...
        let mut pairs = pair.clone().into_inner();
        // structure is field_prefix? ~ identifier ~ ":" ~ param_type
        let prefix = match pairs.peek().map(|p| p.as_rule()) {
            Some(Rule::field_prefix) => {
                let prefix_pair = pairs.try_next()?; // consume field_prefix
                match prefix_pair.into_inner().next() {
                    Some(inner_pair) => match inner_pair.as_rule() {
                        Rule::index => {
                            let is_unique = inner_pair
                                .into_inner()
                                .peek()
                        ...
                        Rule::bm25_index => FieldPrefix::BM25,
                        _ => FieldPrefix::Empty,
                    },
                    None => FieldPrefix::Empty,
                }
            }
            // Backward compat: old grammar had index? directly
            Some(Rule::index) => {
                let index_pair = pairs.try_next()?;
