...
class TestFanStrengthBFS:
    ...
    async def test_fan_1_gives_near_s_max(self):
        ...
        cfg = ActivationConfig(
            fan_s_max=1.5, spread_max_hops=1, spread_energy_budget=10.0,
        )
    ...
    async def test_high_fan_hub_dampening(self):
        ...
        cfg = ActivationConfig(
            fan_s_max=1.5, spread_max_hops=1,
            spread_energy_budget=100.0, spread_firing_threshold=0.001,
        )
        ...
        bonuses_low, _ = await strategy.spread(
            [("seed", 1.0)], provider_low, cfg,
        )
        bonuses_high, _ = await strategy.spread(
            [("seed", 1.0)], provider_high, cfg,
        )
    ...
    async def test_fan_100_gives_near_zero(self):
        ...
        cfg = ActivationConfig(
            fan_s_max=1.5, fan_s_min=0.0, spread_max_hops=1,
            spread_energy_budget=100.0, spread_firing_threshold=0.001,
        )
    ...
    async def test_fan_s_min_floor(self):
        ...
        cfg = ActivationConfig(
            fan_s_max=1.0, fan_s_min=0.5, spread_max_hops=1,
            spread_energy_budget=100.0, spread_firing_threshold=0.001,
        )
    ...
    async def test_config_s_max_override(self):
        ...
        cfg_low = ActivationConfig(
            fan_s_max=1.0, spread_max_hops=1,
            spread_energy_budget=100.0, spread_firing_threshold=0.001,
        )
        cfg_high = ActivationConfig(
            fan_s_max=3.0, spread_max_hops=1,
            spread_energy_budget=100.0, spread_firing_threshold=0.001,
        )

        strategy = BFSStrategy()
        bonuses_low, _ = await strategy.spread(
            [("seed", 1.0)], provider, cfg_low,
        )
        bonuses_high, _ = await strategy.spread(
            [("seed", 1.0)], provider, cfg_high,
        )
...
class TestFanStrengthPPR:
    ...
    async def test_ppr_with_fan_attenuation(self):
        ...
        cfg = ActivationConfig(
            spreading_strategy="ppr", fan_s_max=1.5,
            ppr_expansion_hops=1, ppr_max_iterations=20,
        )
        provider = MockNeighborProvider({
            "seed": [("n1", 1.0, "KNOWS"), ("n2", 1.0, "KNOWS")],
            "n1": [("n3", 1.0, "WORKS_AT")],
