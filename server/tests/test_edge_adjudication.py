...
from tests.conftest import MockExtractor, _test_helix_config
...
def helix_config(tmp_path):
    ...
    These tests ingest to group ``default`` with recurring "Alice"/"Google"
    facts; on the shared session-wide native dir sibling tests see each other's
    entities and raise spurious conflict_with_existing adjudications. Isolate
    each test on its own dir. Falls back to the shared HTTP lane when native is
    unavailable.
    """
...
class TestEdgeAdjudicationHotPath:
    ...
    async def test_clean_remember_creates_no_requests(self, edge_manager, graph_store):
        ...
        assert requests == []
        assert evidence
        # Narrow-extracted single-mention proper nouns defer for later
        # corroboration rather than committing on the hot path (deliberate
        # extraction-quality gating). The invariant here is that an unambiguous
        # statement raises no adjudication request and flags nothing ambiguous;
        # it is not that every fact commits immediately.
        assert all(row["status"] in {"committed", "deferred"} for row in evidence)
    ...
    async def test_ambiguous_remember_creates_request_and_defers_relationship(
        ...
        # Only the ambiguous relationship is adjudicated; entity evidence is
        # never flagged pending (narrow-extracted proper nouns defer rather than
        # commit, but they do not require adjudication).
        assert all(row["status"] != "pending" for row in evidence if row["fact_class"] == "entity")
...
class TestEdgeAdjudicationPhase:
    ...
    async def test_phase_expires_stale_request(
        ...
        phase = EdgeAdjudicationPhase(graph_manager=edge_manager)
        # The native store exposes no created_at setter, so advance the phase
        # clock past the TTL rather than back-dating the stored request. From
        # `stale_now` the just-created request is older than the 1h TTL.
        stale_now = utc_now() + timedelta(hours=cfg.edge_adjudication_request_ttl_hours + 1)
        with patch(
            "engram.consolidation.phases.edge_adjudication.utc_now",
