...
impl NodeSchema {
    /// Returns field names annotated with `BM25` prefix.
    /// Empty vec means "index all fields" (backward compatible).
    pub fn bm25_fields(&self) -> Vec<String> {
        self.fields
            .iter()
            .filter(|f| f.prefix.is_bm25())
            .map(|f| f.name.clone())
            .collect()
    }
}
