...
class TestLiteModeIntegration:
    async def test_ingest_and_recall(self, graph_manager: GraphManager):
        ...
        results = await graph_manager.recall(query="Python", group_id="default")
        assert len(results) >= 1
        entity_names = [r["entity"]["name"] for r in results if "entity" in r]
        assert "Python" in entity_names

    async def test_multiple_episodes(self, graph_manager: GraphManager):
        ...
        results = await graph_manager.recall(query="FastAPI", group_id="default")
        assert len(results) >= 1

    async def test_entity_dedup(self, graph_manager: GraphManager):
        """Same entity name from multiple episodes should not duplicate."""
        await graph_manager.ingest_episode(content="Working with Python", group_id="default")
        await graph_manager.ingest_episode(content="More Python work", group_id="default")

        # Check that we don't have duplicate entities
        store = graph_manager._graph
        entities = await store.find_entities(name="Python", group_id="default")
        assert len(entities) == 1

    async def test_stats_after_ingestion(self, graph_manager: GraphManager):
        """Stats should reflect ingested data."""
        await graph_manager.ingest_episode(content="Testing stats", group_id="default")
        stats = await graph_manager._graph.get_stats(group_id="default")
    ...
    async def test_fuzzy_dedup_across_episodes(self, graph_store, activation_store, search_index):
        ...
        await manager1.ingest_episode(content="ep1", group_id="default")
        ...
        await manager2.ingest_episode(content="ep2", group_id="default")

        # Should be one entity, not two
        entities = await graph_store.find_entities(group_id="default")
    ...
    async def test_recall_with_activation_scores(self, graph_manager: GraphManager):
    ...
    async def test_ingest_records_access(self, graph_manager: GraphManager):
        ...
        entities = await graph_manager._graph.find_entities(group_id="default")
        for entity in entities:
            state = await graph_manager._activation.get_activation(entity.id)
            assert state is not None
            assert len(state.access_history) >= 1

    async def test_recall_records_access(self, graph_manager: GraphManager):
    ...
    async def test_activation_boosts_frequently_mentioned(
        ...
        for _ in range(3):
            ...
            await mgr.ingest_episode(content="ep", group_id="default")
        ...
        await mgr2.ingest_episode(content="ep", group_id="default")
    ...
    async def test_recall_empty_returns_empty(self, graph_manager: GraphManager):
        """Empty query on empty store still works."""
        results = await graph_manager.recall(query="nonexistent", group_id="default")
        assert results == []

    async def test_recall_scores_descending(self, graph_manager: GraphManager):
        """Results sorted by score descending."""
        await graph_manager.ingest_episode(
            content="Testing score ordering",
            group_id="default",
        )
        results = await graph_manager.recall(query="Python FastAPI", group_id="default")
        if len(results) >= 2:
