...
impl HelixGraphStorage {
    pub fn new(
        ...
        // Register per-label BM25 field filters from config
        if let Some(ref mut bm25_config) = bm25 {
            if let Some(ref filters) = config.bm25_field_filters {
                for (label, fields) in filters {
                    bm25_config.register_bm25_fields(label, fields.clone());
                }
            }
        }
