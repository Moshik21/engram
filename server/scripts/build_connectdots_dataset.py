...
SESSIONS = [
    ("s1", "2024/01/08 (Mon) 09:15", [
        ("user", "Started my new job today at Nimbus Labs. I'm on the Helix project with a teammate named Priya."),
        ("assistant", "Congrats on the new role at Nimbus Labs! Working with Priya on Helix sounds exciting."),
    ]),
    ("s2", "2024/01/20 (Sat) 18:40", [
    ...
    ("s3", "2024/02/03 (Sat) 11:05", [
        ("user", "Spent the day deep in the Helix codebase. It's written in Rust and backed by a Postgres database."),
        ("assistant", "Rust plus Postgres is a solid stack for Helix."),
    ]),
    ("s4", "2024/02/18 (Sun) 14:20", [
        ("user", "I adopted a dog! Her name is Biscuit and she's a beagle."),
        ("assistant", "Welcome to the family, Biscuit the beagle!"),
    ]),
    ("s5", "2024/03/05 (Tue) 10:00", [
        ("user", "Priya is our team lead. She actually used to work at Acme before joining Nimbus."),
        ("assistant", "Good context on Priya — team lead, ex-Acme."),
    ]),
    ("s6", "2024/03/22 (Fri) 19:30", [
    ...
    ("s7", "2024/04/10 (Wed) 16:45", [
        ("user", "Huge week — we shipped the launch. Right after it, Priya was promoted to Director."),
        ("assistant", "Massive milestone, and a well-earned promotion to Director."),
    ]),
    ("s8", "2024/04/25 (Thu) 08:50", [
        ("user", "Rough day: Biscuit had surgery on her leg. The vet says she'll recover fully."),
        ("assistant", "Sorry to hear about the leg surgery — glad Biscuit's expected to fully recover."),
    ]),
    ("s9", "2024/05/12 (Sun) 13:15", [
        ("user", "Turns out Marcus also teaches piano, not just guitar. I'm tempted to switch."),
        ("assistant", "Marcus teaching piano too gives you a nice option to switch."),
    ]),
    ("s10", "2024/05/30 (Thu) 21:10", [
...
QUERIES = [
    ("m1", "multi_hop",
     # anchor: Priya = Helix teammate (s1); answer 'Director' lives in launch/promotion session (s7)
     "What is the current job title of my Helix teammate?",
     "Director", ["s7"]),
    ("m2", "multi_hop",
     # anchor: the pet that had leg surgery = Biscuit (s8); answer 'beagle' is in the adoption session (s4)
     "What breed is the pet of mine that later needed leg surgery?",
     "beagle", ["s4"]),
    ("m3", "multi_hop",
     # anchor: the person I'm going to Japan with = Priya (s10); answer 'Director' is in s7
     "What is the job title of the person I'm planning to travel to Japan with?",
     "Director", ["s7"]),
    ("m4", "multi_hop",
     # anchor: the teacher who also teaches piano = Marcus (s9); answer 'guitar' is in s6
     "Which instrument did I originally sign up to learn from the teacher who also offers piano?",
     "guitar", ["s6"]),
    ("m5", "multi_hop",
     ...
     "Acme", ["s5"]),
    ("c1", "single_hop_control",
     "Do I prefer tea or coffee?", "Tea", ["s2"]),
    ("c2", "single_hop_control",
     "What is my dog's name?", "Biscuit", ["s4"]),
    ("c3", "single_hop_control",
     "Who is my guitar instructor?", "Marcus", ["s6"]),
]
