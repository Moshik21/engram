...
**Change:**
- ~~`config.hx.json:5` `ef_search: 512 → 96` … Requires index reload, no rebuild.~~ **CORRECTED 2026-07-24 (ticket #27): this edit is a no-op.** `config.hx.json` is read by nothing; the effective value is the generated Rust literal `queries.rs:100 ef_search: Some(768)`, which `HelixEngine::new` consumes directly (`helix-python/src/lib.rs:53`) and `NativeTransport.initialize()` never overrides. The real change is **`queries.rs` `fn config()` 768 → 96, mirrored into all three `config.hx.json` copies, plus `make build-native`** — so it is *not* free and *not* reload-only. `server/tests/test_native_config_authority.py` fails if the JSON is edited without the Rust.
...
2. ~~**Is `ef_search` runtime-tunable or index-baked?** `config.hx.json` is read at index open …~~ **ANSWERED-IN-PART 2026-07-24 (ticket #27):** the premise was wrong. `config.hx.json` is **not** read at index open, or anywhere else — the value is compiled into `queries.rs fn config()`. So changing it always requires `make build-native`. The *remaining* open question is narrower and still worth answering: once the binary carries a new `ef_search`, does the existing HNSW index serve it on reload, or does `m`/`ef_construction` need a rebuild of the vectors themselves? (`ef_search` is a query-time knob, so reload-only is the expectation; `ef_construction: 128` is index-baked and would need a re-index.)
