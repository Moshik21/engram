...
NATIVE_SURFACE_MANIFEST: tuple[NativeSurface, ...] = (
    ...
    NativeSurface(
        kind="rest",
        identifier="GET /api/hygiene/debt",
        coverage="static_not_data_bound",
        evidence="server/engram/api/hygiene.py",
        note="Read-only debt scoreboard; the cold brain preflights mop windows here.",
    ),
    NativeSurface(
        kind="rest",
        identifier="GET /api/loop/status",
        coverage="static_not_data_bound",
        evidence="server/engram/api/loop.py",
        note="Loop Steward status (file-first; sidecar is an audit mirror).",
    ),
    NativeSurface(
        kind="rest",
        identifier="POST /api/loop/apply",
        coverage="static_not_data_bound",
        evidence="server/engram/api/loop.py",
        note="Operator apply of a clamped TTL LoopAdjustment.",
    ),
    NativeSurface(
        kind="rest",
        identifier="POST /api/loop/clear",
        coverage="static_not_data_bound",
        evidence="server/engram/api/loop.py",
        note="Operator clear of the active LoopAdjustment.",
    ),
    NativeSurface(
        kind="rest",
        identifier="DELETE /api/loop/status",
        coverage="static_not_data_bound",
        evidence="server/engram/api/loop.py",
        note="Alias of POST /api/loop/clear.",
    ),
    ...
    NativeSurface(
        kind="rest",
        identifier="GET /api/knowledge/runtime/fast",
        coverage="static_not_data_bound",
        evidence="server/engram/retrieval/runtime_state.py",
        note="Startup-safe runtime packet; skips graph/artifact inspection.",
    ),
    NativeSurface(
        kind="rest",
        identifier="GET /api/knowledge/packet-cache",
        coverage="static_not_data_bound",
        evidence="server/engram/retrieval/packet_cache_surface.py",
        note="Operator packet-cache diagnostics; no cache mutation.",
    ),
    NativeSurface(
        kind="rest",
        identifier="POST /api/knowledge/packet-cache/clear",
        coverage="static_not_data_bound",
        evidence="server/engram/retrieval/packet_cache_surface.py",
        note="Operator packet-cache maintenance clear.",
    ),
    NativeSurface(
        kind="rest",
        identifier="GET /api/knowledge/adjudications",
        coverage="static_not_data_bound",
        evidence="server/engram/ingestion/adjudication_surface.py",
        note="Dashboard pending-adjudication queue read.",
    ),
    NativeSurface(
        kind="rest",
        identifier="GET /api/storage",
        coverage="static_not_data_bound",
        evidence="server/engram/api/storage.py",
        note="Operator/dashboard storage visibility report.",
    ),
    ...
    NativeSurface(
        kind="websocket",
        identifier="/ws/ingest",
        coverage="static_not_data_bound",
        evidence="server/engram/api/ingest_ws.py",
        note="Fluid-capture ingest WebSocket transport is route-table distinct from REST.",
    ),
    NativeSurface(
        kind="mcp_transport",
        identifier="/mcp",
    ...
    NativeSurface(
        ...
        identifier="timeline",
        coverage="static_not_data_bound",
        evidence="server/engram/retrieval/timeline.py",
        note="Dated, chronologically-ordered timeline + deterministic temporal planner; read-only.",
    ),
    ...
    NativeSurface(
        kind="mcp_tool",
        identifier="loop_status",
        coverage="static_not_data_bound",
        evidence="server/tests/test_mcp_surface.py",
        note="Loop Steward status (operator surface only; never public).",
    ),
    NativeSurface(
        kind="mcp_tool",
        identifier="loop_apply",
        coverage="static_not_data_bound",
        evidence="server/tests/test_mcp_surface.py",
        note="Loop Steward apply (operator surface only).",
    ),
    NativeSurface(
        kind="mcp_tool",
        identifier="loop_clear",
        coverage="static_not_data_bound",
        evidence="server/tests/test_mcp_surface.py",
        note="Loop Steward clear (operator surface only).",
    ),
    NativeSurface(
        kind="mcp_tool",
        ...
        note=(
            "Re-proposal drain listing: narrow-projected episodes for the resident "
            "agent (operator surface only)."
        ),
    ),
    NativeSurface(
        kind="mcp_tool",
        ...
        coverage="static_not_data_bound",
        evidence="server/tests/test_mcp_surface.py",
        note="Deterministic propose-from-report (operator surface only).",
    ),
    NativeSurface(
        kind="mcp_tool",
        identifier="loop_steward_once",
        coverage="static_not_data_bound",
        evidence="server/tests/test_mcp_surface.py",
        note="One-shot steward pass (operator surface only).",
    ),
