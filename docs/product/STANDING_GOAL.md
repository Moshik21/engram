...
| — | **Step 5: the graph kill rig ran as the experiment — verdict KILL on the planted control (reach@5 A=0, B=0, C=1 of 51; K1, K2, K4 all trip), VOID on the organic producer (zero bridges from real text)** | 2026-09-04, `docs/product/GRAPH_THESIS.md` §5 'Run 2026-09-04', envelopes under `docs/product/artifacts/`. The graph as shipped surfaces the edge and never the episode behind it, at +7.7 rows / +1.5 k chars per recall. Design space (§4 D/E) not killed; the shipped consumer is. |
