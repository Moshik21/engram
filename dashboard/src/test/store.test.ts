...
function makeEvaluationReport(
  ...
  const base: BrainLoopEvaluationReport = {
    ...
    evaluationSignals: {
      cueUsefulness: { status: "measured", evidenceCount: 5, metric: 0.4, gap: null },
      projectionYield: { status: "measured", evidenceCount: 2, metric: 3, gap: null },
      recallQuality: { status: "measured", evidenceCount: 1, metric: 1, gap: null },
      falseRecall: { status: "measured", evidenceCount: 2, metric: 0, gap: null },
      triageCalibration: {
        status: "needs_snapshots",
        evidenceCount: 0,
        metric: null,
        gap: "consolidation calibration needs saved calibration snapshots",
      },
...
describe("evaluationSlice", () => {
  it("loadEvaluationReport stores brain-loop evaluation report", async () => {
    mockedApi.getEvaluationReport.mockResolvedValueOnce(makeEvaluationReport());
