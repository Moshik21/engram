...
> Extended documentation moved from README for a cleaner landing page. Start with the [README](../README.md).
...
- **Harness capture**: When `--capture-transcript` or AutoCapture hooks are installed, routine turns post to `/api/knowledge/auto-observe` (`auto:prompt`, `auto:response`) — no per-turn agent `observe`
- **Agent observe**: Only for explicit store requests or context the harness cannot see (cheap, no LLM; cue-backed when enabled)
...
The system prompt is harness-first: routine capture is infrastructure when hooks are active; agents focus on `get_context`, `recall`, and `remember` for high-signal facts. The background worker and triage phase promote valuable `auto:*` episodes without per-turn agent `observe`.
...
For hook-driven capture, run `engramctl connect claude-code --capture-transcript` or `engram harness install-autocapture`. Fresh installs generate
...
- Harness auto-capture handles routine turns when installed — do not duplicate with per-turn `observe`.
- Use `remember()` for high-signal cross-context facts; use `observe()` only per `agent_protocol.capture` (explicit store or harness-invisible context).
