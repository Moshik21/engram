...
pub struct HBM25Config {
    ...
    k1: f64,
    b: f64,
    /// Per-label BM25 field filters. If a label has an entry here, only those
    /// fields are indexed for BM25. If absent, all fields are indexed (backward
    /// compatible).
    bm25_field_filters: HashMap<String, HashSet<String>>,
}
...
impl HBM25Config {
    pub fn new(graph_env: &Env, wtxn: &mut RwTxn) -> Result<HBM25Config, GraphError> {
        ...
        Ok(HBM25Config {
            ...
            k1: 1.2,
            b: 0.75,
            bm25_field_filters: HashMap::new(),
        })
    }

    /// Register which fields should be BM25-indexed for a given node label.
    /// If called with a non-empty set, only those fields will be tokenized
    /// during indexing. If never called for a label, all fields are indexed.
    pub fn register_bm25_fields(&mut self, label: &str, fields: Vec<String>) {
        if !fields.is_empty() {
            self.bm25_field_filters
                .insert(label.to_string(), fields.into_iter().collect());
        }
    }
    ...
    pub(crate) fn term_counts_for_node(
        ...
        // Check if this label has field-level BM25 filtering
        let filter = self.bm25_field_filters.get(label);

        for (key, value) in properties.iter() {
            // If a filter is registered, only index fields in the whitelist
            if let Some(allowed_fields) = filter {
