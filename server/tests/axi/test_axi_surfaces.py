...
def test_home_payload_adds_metadata_trace_flags_to_read_followups() -> None:
    ...
    assert "--timeout 5" in context_cmd
...
def test_home_payload_reports_busy_runtime_timeout_as_degraded() -> None:
    ...
    assert result.payload["mode"] == "helix"
    assert result.payload["context"]["cmd"] == "engram axi context --budget 800 --timeout 5"
    assert result.payload["next"][0]["cmd"] == "engram axi context --budget 800 --timeout 5"
