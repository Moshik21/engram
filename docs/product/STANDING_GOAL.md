...
| — | **character cost: recall items are windowed around the query-term cluster (`recall_content_window_chars`, 450, measured) — untouched battery 14/14 at 21,751 chars vs 13,335 from repo files (1.63×, bar ≤ 2×; was 59,165 = 4.4×)** | 2026-09-06, `b3350fb` + `01cea12`. Every cut is marked (`fullChars`/`windowed`); short rows untouched. 600 read 31.9k and an offline replay over the rig rows lost 2 groups at 600, 0 at 450 (the densest-cluster choice is not monotonic in the size). No by-id episode read exists on the public surface: the documented recovery is a second recall with the exact phrase. |
