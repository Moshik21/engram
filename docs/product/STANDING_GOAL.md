...
| — | ~~**the session's most recent observe answers any later query that shares ≥3 common tokens with it, as `cache_satisfied`, without running the pipeline**~~ **FIXED `b3350fb` + `01cea12` (2026-09-06)**: session-recent packets satisfy only on a high-signal token or three distinctive matches covering half the query, with stopwords and the project's own header name excluded (the query prefix 'Engram' had counted). Live: the faithful repro reaches the pipeline; a high-signal control still hits the cache. | 2026-09-04, `recall_surface.py` `_packet_query_match` (`best_score >= 3`). Live: 'second capture on the restarted shell with the project field' was answered 7 min later by the *fifth* capture (overlap: capture, project, field) and the real episode never ran through recall. Open: the rule needs a high-signal-token requirement or a recency-scoped intent, and a measurement of how often a real session trips it. |
...
| — | ~~**capture-time episode indexing embeds every chunk of every capture; under load it falls behind and the durable outbox only drains 100 rows per shell start**~~ **FIXED `b3350fb` (2026-09-06)**: continuous drain loop (bounded batches, yields to recall, in-flight snapshot against duplicate rows, counts on `/api/storage` diagnostics.indexOutbox) and capture-time chunk vectors OFF by default (`chunk_vectors_enabled`). Live: 60 rows at 5/s indexed inline, queue never grew. | 2026-09-04 re-seed: 4,223 of 5,898 rows queued; the shell's startup replay is `cue_index_outbox_replay_limit=100`, one-shot, and there is no continuous drain. Measured with the shell down: chunked `index_episode` >2 s/row (3 GB RSS, swapping), single full-content vector 0.15 s/row (787 MB). The July emergency backfill wrote single vectors and the chunk re-index sweep was measured to hurt recall (reindex_sweep_enabled=False), so the backfill wrote single vectors. Open: make the outbox drain continuous (or larger), and decide whether capture-time chunking is worth its cost. |
| — | ~~**project identity was the cwd basename: one repo became 'Engram' + 'server' + 'app' + 'reports', a home-dir cwd became a project named after the user, and the other-project multiplier demoted half of the repo's own rows**~~ **FIXED (resident-agent follow-up, 2026-09-04 night)** | On the fresh store the four stable rig misses were all rows tagged `server` asked from the repo root (2/2 groups with project `server` or none, 0/2 with `Engram`). `ingestion/project_identity.py`: identity = the repository root's name (nearest `.git`); home/root = no project; at recall the accepted set adds the root's immediate subdirectory names so legacy rows still count. Export counts of the split: Engram 531, server 237, app 164, konnermoshier 232, reports 26. |
| — | ~~**two copies of a project post-filter (`recall_surface.py:1865`, `context_builder.py:4363`) kept only rows mentioning the cwd basename and dropped every row captured from a subdirectory whenever one root-tagged row matched**~~ **FIXED `d5ea692` + `cc7b6a5`** | 2026-09-04 night. Found by the fresh store: 49–55 candidates scored, 1–5 rows returned, all tagged `Engram`; the `server`-tagged answers never reached the response. Both copies now accept the repository's name set (`project_identity.accepted_project_names`) whole-word. Untouched rig after: **[14, 14, 14] of 14**, 0/42 degraded, median 109 ms. |
| — | ~~**the cold-brain LaunchAgent ran zero windows in the 42 awake hours after the switch: launchd held its interval spawn as 'pended' because the job is ProcessType Background on a memory-pressured Mac**~~ **FIXED in the templates (2026-09-06); installed plist reload owed** | `launchctl print`: `pended nondemand spawn = interval`, runs unchanged at 22 since the Sep-4 kickstart. A kickstart on 2026-09-06 ran clean on the new store (110 s, shell paused/resumed, backfill missing 0, de-index 0, replay 19 → 8 narrow entities). Templates now `Standard` + `Nice 10`. The earlier 'battery skip' explanation for silent windows was incomplete: this deferral also applies on AC. |
...
| — | **character cost: recall items are windowed around the query-term cluster (`recall_content_window_chars`, 450, measured) — untouched battery 14/14 at 24,332 chars vs 13,335 from repo files (1.82×, bar ≤ 2×; was 59,165 = 4.4×) and the untouched rig 14/14 on every run at 101 ms median** | 2026-09-06, `b3350fb` + `01cea12`. Every cut is marked (`fullChars`/`windowed`); short rows untouched. 600 read 31.9k and an offline replay over the rig rows lost 2 groups at 600, 0 at 450 (the densest-cluster choice is not monotonic in the size). Spread terms stretch the window within a 3× cap (`f53ac01`): at a fixed 450 the rig read 13/14 because one answer's two groups sat ~1,000 chars apart in every top row. No by-id episode read exists on the public surface: the documented recovery is a second recall with the exact phrase. |
...
- **2026-09-04 (night)** — **untouched rig 14/14 on every run** after the two project post-filters were
  fixed (`d5ea692`, `cc7b6a5`): 3–4 → 10 (fresh store) → 12 (identity) → 14 (filters). Same
  code family, same machine, no graph in the store. Day-14 gate on 2026-09-18 decides KEEP; the
  old dir stays until then; the old-brain bar is still owed from it on a quiet machine.
- **2026-09-06** — two days of organic use: +2 captures only (the store was not used). First mop
  window on the new store ran clean after a manual kickstart; the LaunchAgent had been
  `pended` by launchd (Background process type) — templates fixed, installed plist reload owed.
  48 h probe: swap p50 5.1 GB / max 6.2 GB with the shell at 113 MB p50 resident and entity
  reads at 19 ms p50 — the pressure is the machine, not the store.
- **2026-09-06** — three reviewed lanes landed (`b3350fb`, `01cea12`): windowed recall content
  (battery 14/14 at 1.63× repo-file chars, bar met), a continuous index-outbox drain with
  single-vector capture by default (60 rows at 5/s never queued), and the distinctive gate on
  recent-observation packets (live repro reaches the pipeline). Meter pass at 450 in flight.
- **2026-09-06** — final read after the stretch (`f53ac01`): untouched rig **[14, 14]**, 0/42
  degraded, p50 101 ms; battery **14/14 at 1.82×** repo-file chars. Every item from the
  2026-09-06 open list that was not 'use it' is landed except the old-brain bar (machine not
  quiet: swap 5 GB) and Konner's three actions (plist reload, clawhub republish, key rotation).
